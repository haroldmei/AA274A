...
def corr(F, I):
    ...
    for i in range(m):
       for j in range(n):
           for t in range(c):
               padding[(i+(k/2)), (j+(k/2)), t] = I[i, j, t]

    for i in range(m):
        for j in range(n):
            a = padding[i:i+2*(k/2)+1]
            tij = (a[:, j:j + 2 * (l / 2)]) if l%2 == 0 else (a[:, j:j+2*(l/2)+1])
...
def norm_cross_corr(F, I):
    ...
    for i in range(m):
        for j in range(n):
            ...
            tij = (a[:, j:j + 2 * (l / 2)]) if l%2 == 0 else (a[:, j:j+2*(l/2)+1])
            tij = np.reshape(tij, (k*l*c, 1))
            G[i, j] = 0 if np.linalg.norm(tij) == 0 else np.dot(np.array(f), np.array(tij))/(np.linalg.norm(f) * np.linalg.norm(tij))
    return G
    ########## Code ends here ##########
